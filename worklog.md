...
Task: Split the navigation menu into 2 parts (visible primary actions + creative "Explorer" dropdown) and fix ALL non-responsive pages so the platform works on a 390 px (iPhone SE) viewport.

Work Log:

# TASK 1 — Navigation menu split

## Part 1: Primary actions (visible in the nav bar on desktop)
Kept as-is from FIX2:
...
- Admin (ShieldCheck icon, amber) → openAdmin (admin only)

## Part 2: "Explorer" dropdown (replaces "Plus")
- New icon: Compass (replaced Grid).
- New label: "Explorer" (was "Plus").
- Trigger style: bg-gradient-to-r from-emerald-500 to-teal-600 text-white (emerald→teal gradient).
- Dropdown width: w-64 → w-72.
- 4 themed groups with emoji-prefixed labels:
...
- "Ligues" maps to openLeaderboard (no separate openLeagues action; leaderboard view hosts league info).
...
## session-view.tsx
- Top bar gap: gap-3 → gap-2 on mobile.
...
- Buttons cluster: simplified to flex-wrap gap-2.
- Difficulty filter bar Card padding: p-4 → p-3 sm:p-4.
- Difficulty filter buttons: smaller on mobile (px-2 py-1 text-[11px] sm:px-2.5 sm:py-1.5 sm:text-xs).
- Question preview header: px-6 py-4 → px-4 py-3 sm:px-6 sm:py-4 + break-words on description.
- Question preview rows: px-6 py-4 → px-4 py-3 sm:px-6 sm:py-4.
- Question preview meta row: stacks on mobile (flex-col gap-1 sm:flex-row sm:items-center sm:gap-2), ChevronRight hidden on mobile.
- Explanation text: line-clamp-1 → break-words line-clamp-2 sm:line-clamp-1.
...
## admin-view.tsx (and sub-components)
- admin-overview.tsx: StatCard grid gap-3 sm:grid-cols-3 lg:grid-cols-6 → grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6 (2 cols on mobile).
- admin-sessions.tsx: session row stacks on mobile (flex-col gap-2 sm:flex-row sm:items-center sm:justify-between sm:gap-3 sm:px-5). Title and user/email <p> switched from truncate to break-words sm:truncate. Score cluster: added self-end sm:self-auto.
