...
// Map of icon string → component (so we can store icon name in badge)
...
export function AchievementsView() {
  ...
  const visibleBadges = useMemo(() => {
    ...
    if (filter === "locked") list = list.filter((b) => !b.unlocked);
    if (search.trim()) {
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-2 text-xs">
          ...
          <Badge variant="secondary" className="gap-1">
            <BookOpen className="h-3 w-3" /> {totalAnswered} Q répondues
          </Badge>
        </div>
      </div>
      ...
      <Card className="p-5">
        <div className="mb-2 flex items-center justify-between text-sm">
          ...
          <span className="text-muted-foreground">
            {unlockedCount} / {badges.length} ({progressPct}%)
          </span>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5">
          {visibleBadges.map((badge) => {
            ...
            return (
              <Card
                key={badge.id}
                className={`group relative flex flex-col items-center gap-2 p-4 text-center transition-all hover:scale-[1.02] ${
                  badge.unlocked
                    ? "border-amber-300 bg-gradient-to-br from-amber-50 to-yellow-50 dark:border-amber-700 dark:from-amber-950/30 dark:to-yellow-950/20"
              ...
              >
                ...
                {badge.unlocked && (
                  <Badge
                    variant="outline"
                    className="absolute right-1.5 top-1.5 border-amber-300 bg-amber-100 px-1 text-[9px] text-amber-700 dark:border-amber-700 dark:bg-amber-950/50 dark:text-amber-300"
                  >
                    ✓
                  </Badge>
>>>>>>> Stashed changes
